...
export async function app() {
    try { 
        ...
        app.use(async (req, res, next) => {
            ...
            await db.add('analytics.allRequests', 1) // Requests - Logs all traffic
            ...
            if(!(req.path).includes('.js') && !(req.path).includes('.css') && !(req.path).includes('.ico') && !(req.path).includes('api')){
                ...
                await db.add('analytics.hiddenRequests', 1) // Requests - Hidden Traffic
            }
        ...
        app.get('/:path', async (req, res) => {
            ...
            if (paths && paths[path]) {
                await db.add('analytics.pageVisits', 1) // Path Visit
                await db.add('')
